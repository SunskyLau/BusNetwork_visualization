<script>
  ...
  $: if (activeIndex > 0) {
    relChangePrevDecade = relChange(
      data[activeIndex - 1].population,
      data[activeIndex].population
    );
    relChangeFirstDecade = relChange(
      data[0].population,
      data[activeIndex].population
    );
  }

  const widthScale = scaleLinear().domain([0, 7470040]).range([0, 407]);
</script>
...
<div class="graph">
  ...
  <div class="main-chart">
    ...
    <svg id="chart" width="900" height="600">
      {#each data as item, i}
        <g>
          <text class="graph-text" x={yearXPosition} y={yPos(i)} dy={10}
            >{item.year}</text
          >
          <text class="graph-text" x={separatorXPosition} y={yPos(i)} dy={10}
            >–</text
          >
          <text
            class="graph-text"
            x={populationXPosition}
            y={yPos(i)}
            dy={10}
            text-anchor="end"
          >
...
<div class="caption">
  ...
  {#if activeIndex > 0}
    <p>
      ...
      {relChangePrevDecade}% compared with the population 10 years earlier; and
      a
      {relChangeFirstDecade}% increase compared with the population of
      African-Americans in 1750.
    </p>
  ...
  {:else}
    <p>Hover over each rectangle to find out more...</p>
  {/if}
</div>
